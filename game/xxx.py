...
def start():
    ...
    if session['player2'] == 'computer':
        ...
        # session['figure'] = player.random_figure(game.figures)
...
def play():
    ...
    if isinstance(player, quatro.Computer):
        # figure = game.select_from_figures(session['figure'])
        symbol = player.random_figure(game.figures)
        figure = player.select_from_figures(symbol, game.figures)
    if figure:
    ...
    print(name, player.flag, player.turn, player.flag)
    if winner:
        game.save_winner(winner)
        del game

    return render_template('board2.html', board2=current_board2, board=current_board, figures=player.current_figures,
                           symbols=symbols,
